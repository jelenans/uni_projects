        ...
        public List<State> mogucaSledecaStanja()
        {
           ...
           /* for (int i = 1; i < Gradovi.cBrojGradova; i++)
            ...
            for (int i = 0; i < Lista.listaRastojanja.Count; i++)
            {
                PictureBox a=null;
                nemojDodati = false;
                if (grad.Tag.Equals(Lista.listaRastojanja[i].g1.Tag))
                    a = Lista.listaRastojanja[i].g2;
                if (grad.Tag.Equals(Lista.listaRastojanja[i].g2.Tag))
                    a = Lista.listaRastojanja[i].g1;
                if(a!=null){  
                   for(int j=0;j<gradovi.Count;j++)
                       if (gradovi[j].Tag.Equals(a.Tag))
                            nemojDodati=true;
                   if (!nemojDodati)
                       rezultat.Add(sledeceStanje(a, Lista.listaRastojanja[i].razd));
